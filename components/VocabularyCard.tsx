...
const VocabularyCard: React.FC<VocabularyCardProps> = ({ word, index }) => {
  ...
  return (
    <div
      ...
      className={`group relative h-44 cursor-pointer rounded-2xl border border-soft bg-surface-soft text-primary shadow-soft transition-all duration-500 ease-out [perspective:1200px] ${
        inView ? 'translate-y-0 opacity-100' : 'translate-y-6 opacity-0'
      } hover:-translate-y-1 focus:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--accent)] focus-visible:ring-offset-2`}
    >
      <div
